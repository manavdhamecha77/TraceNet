...
| 2.5 | CLIP embedding extraction per tracklet (best crop or averaged) | Backend/Embeddings | done | Verified CLIP singleton loads on FastAPI startup via `TestClient(app)` and a direct `get_clip_encoder()` smoke test. |
