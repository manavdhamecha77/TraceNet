...
| 4.4 | Loitering / dwell-time detection (track_id stays in defined zone beyond threshold) | Backend/Alerts | in progress | Split into zone configuration, dwell analysis, and alert presentation so each part can be verified end-to-end. |
| 4.4a | Video-scoped polygon-zone configuration, preview-frame API, and zone editor | Backend + Frontend | in progress | Zones use normalized coordinates against the standardized video frame; threshold is configurable per analysis. |
| 4.4b | Dwell-time analysis using bottom-centre track points, grace gaps, and alert deduplication | Backend/Alerts | in progress | Analyzer is implemented and synthetic-verified; real-video end-to-end verification remains. |
| 4.4c | Loitering alert presentation, playback, acknowledgement, and end-to-end verification | Frontend + Backend | in progress | Added a dedicated Alerts tab; playback and real-video verification remain. |
