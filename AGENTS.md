...
| 4.4b | Dwell-time analysis using bottom-centre track points, grace gaps, and alert deduplication | Backend/Alerts | not started | Runs after tracking once a video zone has been saved. |
| 4.4c | Loitering alert presentation, playback, acknowledgement, and end-to-end verification | Frontend + Backend | not started | Extends the existing Alerts surface without auto-declaring intent. |
